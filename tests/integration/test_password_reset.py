...
@pytest.mark.asyncio
async def test_forgot_password_returns_204_for_known_email(async_client, test_family_and_user):
    """Always returns 204 — no information leakage about whether the email exists."""
    response = await async_client.post(
        "/v1/auth/forgot-password",
        json={"email": test_family_and_user["email"]},
    )
...
@pytest.mark.asyncio
async def test_forgot_password_sends_reset_link(async_client, test_family_and_user, capsys):
    ...
    assert test_family_and_user["email"] in output
    link_line = next(line for line in output.splitlines() if line.startswith("Link:"))
    assert "token=" in link_line


@pytest.mark.asyncio
async def test_reset_password_success(async_client, test_family_and_user, capsys):
    ...
    login_response = await async_client.post(
        "/v1/auth/login",
        json={"email": test_family_and_user["email"], "password": "NewPassword456!"},
    )
    assert login_response.status_code == 200


@pytest.mark.asyncio
async def test_reset_password_token_is_single_use(async_client, test_family_and_user, capsys):
